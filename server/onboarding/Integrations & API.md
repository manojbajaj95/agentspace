...
If your team is using Slack to communicate then you’ll definitely want to enable our [Slack Integration](/settings/integrations/slack) to get instant link unfurling for AgentSpace documents and access to the `/outline` slash command to search your knowledge base directly from Slack.

## API

Have some technical skills? AgentSpace is built on a fully featured RPC-style [API](https://www.getoutline.com/developers). Create (or even append to) documents, collections, provision users, and more programmatically. All documents are edited and stored in markdown format – try out this example CURL request to get started:
...
curl -XPOST -H "Content-type: application/json" -d '{
  ...
  "token": "API_TOKEN", // get an API token from Settings → API Tokens
  "publish": true
}' 'https://YOUR_INSTALLATION/api/documents.create'
```
